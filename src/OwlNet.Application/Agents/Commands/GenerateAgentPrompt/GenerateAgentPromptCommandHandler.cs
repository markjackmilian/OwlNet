...
/// <summary>
/// Handles the <see cref="GenerateAgentPromptCommand"/> by building a conversation context,
/// calling the LLM via <see cref="ILlmChatService"/>, and parsing the response into either
/// follow-up questions or a generated agent markdown definition.
/// </summary>
public sealed partial class GenerateAgentPromptCommandHandler
    : IRequestHandler<GenerateAgentPromptCommand, ValueTask<Result<AgentGenerationResponseDto>>>
{
    ...
    private readonly ILogger<GenerateAgentPromptCommandHandler> _logger;

    /// <summary>
    /// Initializes a new instance of the <see cref="GenerateAgentPromptCommandHandler"/> class.
    /// </summary>
    /// <param name="llmChatService">The LLM chat service for sending completion requests.</param>
    /// <param name="logger">The logger instance.</param>
    public GenerateAgentPromptCommandHandler(
        ILlmChatService llmChatService,
        ILogger<GenerateAgentPromptCommandHandler> logger)
    {
    ...
    public async ValueTask<Result<AgentGenerationResponseDto>> Handle(
        GenerateAgentPromptCommand request,
        CancellationToken cancellationToken)
    {
        _logger.LogInformation(
            "Generating agent prompt for {AgentName} (type: {AgentType}, forceGenerate: {ForceGenerate}, historyCount: {HistoryCount})",
        ...
        {
            _logger.LogError("LLM chat completion failed for agent {AgentName}: {Error}",
        ...
        _logger.LogDebug("LLM response received for agent {AgentName} ({ResponseLength} chars)",
            request.AgentName, responseText.Length);

        var dto = ParseLlmResponse(responseText);

        _logger.LogInformation(
            "Agent prompt generation completed for {AgentName} with response type {ResponseType}",
    ...
    /// </summary>
    private static List<ChatMessage> BuildMessageList(GenerateAgentPromptCommand request)
    {
    ...
    /// <summary>
    /// Builds the initial user message that provides the LLM with the agent's type, name,
    /// and description context.
    /// </summary>
    private static string BuildInitialUserMessage(GenerateAgentPromptCommand request)
    {
        var message = $"""
    ...
    private sealed class LlmAgentResponse
    {
        ...
        /// <summary>
        /// The generated agent markdown content when <see cref="Type"/> is <c>"markdown"</c>.
        /// </summary>
