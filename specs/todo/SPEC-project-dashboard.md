...
When a user selects a project from the topbar selector (see SPEC-project-crud), they need a landing page that contextualizes the application to that project. This dashboard serves as the project's home — a visual summary of project health and activity.
...
The dashboard also hosts the **favorite toggle** for the project, which is needed by SPEC-project-favorites.
...
- **SPEC-project-crud** — Project entity, CQRS infrastructure, topbar selector, and active project context must exist.
- MudBlazor charting components (`MudChart`).
- DispatchR (established by SPEC-project-crud).
